...
async function checkToken(token) {
  ...
  const resOrError = await axios.get(endpoint).catch((error) => {
    ...
    if (error.response.status === 401) {
      return output.errorText("This API key isn't valid.");
    }
...
async function collectToken() {
  ...
  const breadcrumbs = `${blue('beta.dittowords.com')} > ${blue('Account')} > ${blue('User')}`;
  const tokenDescription = `We'll need you to supply your API key for Ditto.

You can find this at: ${apiUrl}.

${breadcrumbs} under "${chalk.bold('API Keys')}".
`;
...
async function collectAndSaveToken() {
  ...
  output.write("Thanks, that API key works.  We'll save it to:");
  console.log('\n');
