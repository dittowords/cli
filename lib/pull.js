...
function cleanProjectName(projectName) {
  return projectName.replace(/\s/g, "-").toLowerCase().trim();
}
...
async function downloadAndSaveVariant(variantApiId, projects, format, token) {
  ...
  if (["flat", "structured"].includes(format)) {
    const projectsWithDedupedNames = getProjectsWithDedupedNames(projects);

    const savedMessages = await Promise.all(
      projectsWithDedupedNames.map(async ({ id, name }) => {
 ...
 */
function generateJsDriver(variants, format) {
  const fileNames = fs.readdirSync(consts.TEXT_DIR);
...
async function downloadAndSave(projectConfig, token) {
  ...
  try {
    ...
    msg += variants
      ? await downloadAndSaveVariants(projects, format, token)
      : await downloadAndSaveBase(projects, format, token);

    msg += generateJsDriver(variants, format);
