...
## Options
`-m, --meta <data...>` Optional metadata to send arbitrary data to the backend. Ex: `-m context:github-action trigger:manual`. Currently, this option should only be utilized when the CLI is ran within a GitHub action (see [Ditto GitHub Action](https://github.com/dittowords/ditto-github-action)) via `context:github-action`.
