...
/**
 * @param {{githubActionRequest: boolean }} options
 */
async function downloadAndSaveVariants(projects, format, token, options) {
  const githubActionRequest = (options && options.githubActionRequest) || false;
...
async function downloadAndSave(sourceInformation, token, options) {
  ...
  try {
    ...
    msg += variants
      ? await downloadAndSaveVariants(validProjects, format, token, options)
      : await downloadAndSaveBase(validProjects, format, token, options);
...
function pull(options) {
  const meta = options ? options.meta : null;
  const token = config.getToken(consts.CONFIG_FILE, consts.API_HOST);
  const sourceInformation = config.parseSourceInformation();

  if (meta && meta.context === 'github-action') {
    return downloadAndSave(sourceInformation, token, { githubActionRequest: true });
  }
