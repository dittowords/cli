...
type IOSStringsDictAPIData = {
  textItemsMap: TextItemsMap;
  componentsMap: ComponentsMap;
};

export default class IOSStringsDictFormatter extends applyMixins(
  BaseFormatter<IOSStringsDictOutputFile<{ variantId: string }>, IOSStringsDictAPIData>) {
   ...
   */
  protected transformAPIData(data: IOSStringsDictAPIData) {
    Object.entries(data.textItemsMap).forEach(([projectId, projectVariants]) => {
      Object.entries(projectVariants).forEach(([variantId, iosStringsDictFile]) => {
        ...
        this.outputFiles[fileName] ??= new IOSStringsDictOutputFile({
          ...
          content: iosStringsDictFile
        });
      });
    });

    Object.entries(data.componentsMap).forEach(([variantId, iosStringsDictFile]) => {
