...
async function collectProject(initialize) {
...
async function collectAndSaveProject(initialize = false) {
  const project = await collectProject(initialize);
  output.nl();
  output.write(`Thanks for choosing a project.  We'll save this info to: ${output.info(consts.PROJECT_CONFIG_FILE)}`);
  output.nl();
