...
async function checkToken(token) {
  ...
  const resOrError = await axios.get(endpoint).catch((error) => {
    ...
    return output.warnText('We hit an error while querying the API: ', error);
  }).catch(() => output.errorText('We hit an error while querying the API.'));
...
async function collectToken() {
  ...
  const breadcrumbs = `${blue('Account')} > ${blue('User')}`;
  const tokenDescription = `We'll need you to supply your API key for Ditto. You can find this at: ${apiUrl} > ${breadcrumbs} under "${chalk.bold('API Keys')}".`;
...
async function collectAndSaveToken() {
  ...
  output.nl();
  console.log(`Thanks, that key works.  We'll save it to: ${output.info(consts.CONFIG_FILE)}`);
